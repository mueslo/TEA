	...
	if (QSqlDatabase::database("mdb").open())
		{
			...
			mdbquery.exec(	"CREATE TABLE IF NOT EXISTS zoom"+QString::number(i)+" ("
                        ...
                        mdbquery.exec( "PRAGMA synchronous=OFF");
			}
			qDebug("mdb success");
				return true;
		} else return false;
...
//returns false if QSqlDatabase::open() failed
{
	...
	mdb.setDatabaseName("maps.db");
        ...
        if (!adb.isValid()) qDebug("Error: adb is not valid");
        if (!rdb.isValid()) qDebug("Error: rdb is not valid");
        if (!mdb.isValid()) qDebug("Error: mdb is not valid");
...
QSqlQuery getCurrentlyLoadedRoutes()
{
	...
	adbquery.exec("SELECT auid FROM active_metadata");
        if (!adbquery.isActive()) qDebug("aDB error (db)");
	return adbquery;
}
...
bool routePresentInADB(QString checksum)
{
	...
	adbquery.exec();
        if (!adbquery.isActive()) qDebug("aDB error (importing)");
...
bool rrouteLoadedInADB(QString uid)
{
	...
	adbquery.exec();
        if (!adbquery.isActive()) qDebug("aDB error (loading)");
	if (adbquery.next()) return true; else return false;
}
...
bool routePresentInRDB(QString checksum)
{
	...
	rdbquery.exec();
        if (!rdbquery.isActive()) qDebug("rDB error (importing)");
...
QString importRoute(QString TEAFilePath)
{
	...
	adbquery.exec("SELECT max(next_auid) from settings");
        adbquery.first(); qDebug("Next AUID: "+adbquery.record().value(0).toString());
...
void AddTileToDBThread::run()
{
    ...
    if(!mdbquery.first()) //Check if tile does not yet exist in db
    {
        ...
        mdbquery.prepare(qPrintable(	"INSERT INTO zoom"+QString::number(z)+" VALUES("
                                        +QString::number(x)+","
                                        +QString::number(y)+","
                                        +"?"+","
                                        +MapSource+")"));
        mdbquery.addBindValue(*Tile);
...
void addTileToDB(int z, int x, int y, QByteArray *Tile, QString MapSource)
{
    QSqlQuery mdbquery(QSqlDatabase::database("mdb"));
...
QByteArray* getTileFromDB(int zoomLevel, int tileX, int tileY, QString mapsource)
{
	...
	{
            return new QByteArray((mdbquery.record().value(2).toByteArray()));;
	}
